...
def format_order_detail_message(order: Dict[str, Any]) -> str:
    ...
    if is_post:
        method_line = "📮 <b>نحوه ارسال:</b> پست پیشتاز (تسویه کامل ۱۰۰٪ - بدون بیعانه)\n"
        payment_line = f"💳 <b>مبلغ فاکتور (تسویه کامل ۱۰۰٪):</b> {tot_val:,} تومان\n" if tot_val > 0 else f"💳 <b>مبلغ تسویه کامل:</b> {deposit} تومان\n"
    else:
        method_line = "🚚 <b>نحوه ارسال:</b> باربری اختصاصی (بیعانه + تسویه در محل)\n"
        payment_line = f"💳 <b>مبلغ بیعانه پیش‌پرداخت:</b> {deposit} تومان\n"
    ...
    if status == "Awaiting_Payment" and created_str:
        try:
            ...
            if remaining_sec > 0:
                ...
                deadline_notice = (
                    f"⏱ <b>مهلت باقی‌مانده جهت {term_title}:</b> {rem_h} ساعت و {rem_m} دقیقه\n"
                    f"⚠️ <i>توجه: در صورت عدم پرداخت ظرف ۵ ساعت پس از ثبت پیش‌فاکتور، سفارش به صورت خودکار لغو خواهد شد.</i>\n\n"
                )
            else:
                status = "Cancelled"
        except Exception:
    ...
    if status == "Awaiting_Payment":
        if is_post:
            timeline = (
                "📍 <b>وضعیت کنونی: ⏳ در انتظار پرداخت و تسویه کامل وجه</b>\n\n"
                f"{deadline_notice}"
        ...
        if is_post:
            timeline = (
                "📍 <b>وضعیت کنونی: 📮 تایید نهایی و آماده‌سازی ارسال با پست پیشتاز</b>\n\n"
                "🟢 <b>۱. صدور پیش‌فاکتور دیجیتال:</b> تایید شد\n"
                ...
                "🛡 <b>تعهد سلامت هوشمند کالا:</b> کلیه مرسولات پستی دارای بیمه سلامت فیزیکی بوده و تا زمان دریافت صحیح تحت پوشش هستند."
            )
        else:
            timeline = (
        ...
        timeline = (
            "📍 <b>وضعیت کنونی: ✅ تحویل داده شد و سفارش تکمیل گردید</b>\n\n"
            "🟢 <b>۱. صدور پیش‌فاکتور دیجیتال</b>\n"
            f"🟢 <b>۲. واریز وجه ({'تسویه کامل' if is_post else 'پرداخت بیعانه'}) و تایید مالی</b>\n"
            "🟢 <b>۳. بیمه بار و خروج از انبار</b>\n"
            f"🟢 <b>۴. حمل با {'پست پیشتاز' if is_post else 'باربر به مقصد خریدار'}</b>\n"
            f"🟢 <b>۵. {'تحویل قطعی مرسوله' if is_post else 'تست سلامت فیزیکی، تحویل نهایی و تسویه مانده وجه'}</b>\n\n"
            "🌹 <i>از خرید و اعتماد ارزشمند شما به مجموعه هوشمند کالا صمیمانه سپاسگزاریم.</i>"
        )
    elif status == "Cancelled":
        timeline = (
            "📍 <b>وضعیت کنونی: ❌ سفارش به صورت خودکار لغو شد</b>\n\n"
            f"⚠️ <b>علت لغو:</b> عدم {term_title} ظرف مهلت قانونی ۵ ساعت پس از ثبت سفارش.\n\n"
